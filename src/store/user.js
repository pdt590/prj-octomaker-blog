...
export const actions = {
  async loadUser({ dispatch, commit, getters }, userId) {
  ...
  async loadAuthUser({ dispatch, commit, getters }, userId) {
    commit("setAuthLoading", true);
    try {
      const userData = await usersRef.child(userId).once("value");
      const userObject = userData.val();
  ...
  async updateUserPassword({ dispatch, commit, getters }, payload) {
    commit("setAuthLoading", true);
    try {
      const confirmPassword = payload.confirmPassword;
  ...
  async isCorrectPassword({ dispatch, commit, getters }, confirmPassword) {
    try {
      const loadedUser = getters.user;
      const authUser = firebase.auth().currentUser;
      if (authUser) {
   ...
   ** Begin auth actions
   */
  async signUserUp({ dispatch, commit, getters }, payload) {
    commit("setAuthLoading", true);
    try {
  ...
  async logUserIn({ dispatch, commit, getters }, payload) {
    commit("setAuthLoading", true);
    try {
  ...
  async resetUserPassword({ dispatch, commit, getters }, comfirmedEmail) {
    commit("setAuthLoading", true);
    try {
  ...
  async handleResetPassword({ dispatch, commit, getters }, payload) {
    commit("setAuthLoading", true);
    try {
      const auth = firebase.auth();
  ...
  async handleVerifyEmail({ dispatch, commit, getters }, actionCode) {
    commit("setAuthLoading", true);
    try {
      const auth = firebase.auth();
